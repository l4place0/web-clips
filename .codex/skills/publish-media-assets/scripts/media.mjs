...
export async function loadMediaConfig(rootInput) {
  ...
  const configPath = process.env.WEB_CLIPS_MEDIA_CONFIG || fileURLToPath(new URL("../resources/media.config.json", import.meta.url))
  let config
  try {
  ...
  if (
    ...
    throw new MediaPublishError("E_MEDIA_CONFIG", "user-level media.config.json is invalid")
  }
  return { root, config }
}
